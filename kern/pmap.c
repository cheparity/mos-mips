...
void mips_detect_memory() {
	...
	/*
	The value of memory size is stored in a variable called memsize.
	The u_int data type is an unsigned integer type that can store 32-bit values.
	The volatile keyword is used to indicate that the value of the variable can change at any
	time. DEV_MP: Device Mapper, 将多个物理设备映射为一个逻辑设备.
	*/
	/*
	KSEG1 = 1010 0000 0000 0000 0000 0000 0000 0000
	DEV_MP_ADDRESS = 0001 0001 0000 0000 0000 0000 0000 0000
	DEV_MP_MEMORY =  0000 0000 0000 0000 0000 0000 1001 0000
	*/
	...
	npage = memsize / BY2PG;
	// npage = memsize >> PGSHIFT; 本质上和上面是一样的
...
void *alloc(u_int n, u_int align, int clear) {
	/*end对应虚拟地址 0x80400000(kseg0的起始地址),意思是要从end开始分配,根据映射规则，0x80400000
	 *对应的物理地址是 0x400000. 在物理地址 0x400000
	 *的前面，存放着操作系统内核的代码和定义的全局变量或数组.
	 *因为end是在mips定义的,所以拿过来是一个字符串,要转成u_long才能用*/
	extern char end[];
	u_long alloced_mem; // 最后return的地址

	/* Initialize `freemem` if this is the first time. The first virtual address that the
	 * linker did *not* assign to any kernel code or global variables. */
	if (freemem == 0) {
		/*freemem 是一个全局变量，初始值为 0，第一次调用该函数将其赋值为 end。
		这个变量的意义是小于 freemem 对应物理地址的物理内存都已经被分配了。*/
		freemem = (u_long)end;
	}

	/* Step 1: Round up `freemem` up to be aligned properly */
	freemem = ROUND(freemem, align); // 向上取整对齐,对不齐的地址不要了.

	/* Step 2: Save current value of `freemem` as allocated chunk. */
	alloced_mem = freemem; // 最后分配的地址是对齐之后的

	/* Step 3: Increase `freemem` to record allocation. */
	freemem = freemem + n; // 已经分配的地址+n字节
	...
	if (clear) {
		memset((void *)alloced_mem, 0, n); // 如果clear有效,则把分出来的地址清零.
	}
...
/* Overview:
    ...
    使用 alloc 函数为内核一级页表分配 npage 页物理内存
   Hint:
 ...
 * 将页表串起空闲页控制块，这杨就得到了page_free_list链表
 *
 ...
 *   将page_free_list空闲链表头部页控制块对应的物理页面分配出去,将其从空闲链表中移除,并清空
 *   对应的物理页面.最后将pp指向的空间赋值为这个页控制块的地址.
 * Post-Condition:
...
int page_alloc(struct Page **new) {
	...
	memset((void *)page2kva(pp), 0, BY2PG); // 我们操作的都是虚拟地址
	*new = pp;
	return 0;
}
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	/*为什么要加上PDX(va)?这是因为我们需要找到虚拟地址对应的页目录项。页目录索引是虚拟地址的高位部分，它决定了我们应该在哪个页目录项中查找相应的页表。因此，我们需要将页目录指针加上页目录索引来获取相应的页目录项。*/
	...
	/*
	1.
	需要获得页表pgtable,从页目录项pgdir_entryp中查到.取*pgdir_entryp就是查页目录项的内容,即页表的物理页号.
	2. 用PTE_ADDR获取页表的物理页号(把低12位清零,为什么?因为低12位是权限位!).
	3.
	由于不能直接访问物理地址,所以还需要用KADDR把页表的物理地址转化为虚拟地址.并且强转为Pte*类型.
	*/
	/*
	1. 如果页目录项无效（即 (*pgdir_entryp & PTE_V) == 0），则根据 create
	参数的值来决定是否创建一个新的页表。
	*/
	if ((*pgdir_entryp & PTE_V) == 0) {
		if (create == 0) {
			*ppte = 0;
			...
			/*
			2. 如果要创建一个新的页表, 使用 page2pa
			函数将物理页的地址转换为物理地址，并与权限位（PTE_D |
			PTE_V）按位或，然后将结果存储在页目录项中。
			*/
			if (page_alloc(&pp) < 0)
				return -E_NO_MEM;
			pp->pp_ref++;
			pgtable = (Pte *)page2kva(pp);
			*pgdir_entryp = page2pa(pp) | PTE_D | PTE_V;
		}
	} else {
		// 如果这一虚拟地址对应的页目录项存在
		// 则继续访问页目录项对应的二级页表,返回虚拟地址对应的页表项地址
	...
	/*
	PTX(va) 是虚拟地址 va
	的页表索引，它用于计算虚拟地址对应的页表项在页表中的偏移量。因此，pgtable + PTX(va)
	得到的是虚拟地址对应的页表项在内核虚拟地址空间中的地址。
	*/
	*ppte = pgtable + PTX(va);
	return 0;
}
