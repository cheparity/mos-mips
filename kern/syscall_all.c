...
int sys_set_path(char *path) {
	...
	try(envid2env(curenv->env_parent_id, &father_env, 0));
	strcpy(father_env->env_path, path); // 修改父进程
	strcpy(curenv->env_path, path);	    // 修改子进程
	return 0;
}
...
int sys_set_tlb_mod_entry(u_int envid, u_int func) {
	...
	try(envid2env(envid, &env, 1)); /*TODO 这里checkperm参数可能有问题*/
...
int sys_exofork(void) {
	...
	/* Step 3: Set the new env's 'env_tf.regs[2]' to 0 to indicate the return value in
	   child. */
	/* Exercise 4.9: Your code here. (3/4) */
	e->env_tf.regs[2] = 0;
	strcpy(e->env_path, curenv->env_path); // 复制目录
...
void *syscall_table[MAX_SYSNO] = {[SYS_putchar] = sys_putchar,
...
void do_syscall(struct Trapframe *tf) {
	...
	tf->regs[2] = func(arg1, arg2, arg3, arg4, arg5); // v0寄存器是regs[2]
}
