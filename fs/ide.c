...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		// 物理地址0x13000010，DEV_DISK_ADDRESS
		uint32_t temp = diskno;
		/* Exercise 5.3: Your code here. (1/2) */
		panic_on(syscall_write_dev(&temp, DEV_DISK_ADDRESS | DEV_DISK_ID, 4)); // 选择id
		temp = begin + off;						       // 偏移
		panic_on(syscall_write_dev(&temp, DEV_DISK_ADDRESS | DEV_DISK_OFFSET, 4));
		temp = DEV_DISK_OPERATION_READ; // 状态read
		...
		if (temp == 0) {
			panic_on("wrong in reading disk");
		}
		// 把缓冲区的内容读到dst+off中
...
void ide_write(u_int diskno, u_int secno, void *src, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		panic_on(syscall_read_dev(&temp, DEV_DISK_ADDRESS | DEV_DISK_STATUS, 4));
		if (temp == 0) {
			panic_on("wrong in writing disk");
		}
	}
}
